...
On Vercel it uses `@vercel/functions`'s `ipAddress()`, which reads the IP Vercel's own edge
network recorded for the connection — not a client-controlled header parsed by hand. A raw
`x-forwarded-for` read directly from `request.headers` would be spoofable by the client; this
is why the app never does that in production. The local-dev fallback (reading
`x-forwarded-for` directly) only activates when `NODE_ENV !== "production"`, so it can never
be hit by real Vercel traffic.
...
Vercel. Configured limits (`RATE_LIMITS` in the same file): Attendance ID attempts, admin
